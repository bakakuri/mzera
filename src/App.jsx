...
import {
  ...
  Sun, Moon, ShoppingBag, Tag, Star, Eye, Navigation, Users, Film, Mic, Play, Pause, Smile, FileText, Download, UserPlus, Trophy
} from "lucide-react";
...
function CreateSheet({ onClose, onPost }) {
    ...
    <div className="fixed inset-0 z-[60] flex sm:items-center justify-center items-end" style={{ background: "rgba(6,7,12,.55)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full sm:max-w-[520px] sm:rounded-3xl rounded-t-3xl max-h-[90vh] overflow-y-auto" style={{ background: C.surface, boxShadow: SH.pop }}>
        ...
        <div className="px-4 pb-5 pt-1">
          ...
          {!poll && <div className="flex gap-2">{STOCK.map(s => <button key={s} onClick={() => setPicked(s)} className="rounded-xl overflow-hidden shrink-0 transition" style={{ width: 64, height: 64, outline: picked === s ? `2.5px solid ${C.accent}` : "none", outlineOffset: 2 }}><Pic src={img(s, 128, 128)} className="w-full h-full" /></button>)}</div>}
        </div>
      </div>
    </div>
  );
...
function PeoplePicker({ title, cta, exclude = [], onClose, onConfirm }) {
  const [sel, setSel] = useState([]);
  const avail = Object.values(USERS).filter(u => u.id !== ME && !exclude.includes(u.id));
...
function Messages({ convos, openId, setOpenId, onSend, onReply, onCreateConvo, onOpenProfile }) {
  ...
  const afterSend = (id) => { setTyping(true); setTimeout(() => { setTyping(false); onReply(id); }, 1300); };
  ...
  if (cv) {
    ...
    return (
      <div className="fixed inset-0 z-40 flex justify-center" style={{ background: C.paper }}>
        ...
        {picker === "add" && <PeoplePicker title="ჩატში დამატება" cta="დაამატე" exclude={members} onClose={() => setPicker(null)} onConfirm={startAdd} />}
      </div>
    );
  }
  ...
  return (
    <div className="pb-28 md:pb-8">
      ...
      {picker === "new" && <PeoplePicker title="ახალი ჩატი" cta="შექმნა" onClose={() => setPicker(null)} onConfirm={startNew} />}
    </div>
  );
}
...
export default function App() {
  ...
  const toggleFollow = (id) => { const now = following.includes(id); setFollowing(f => now ? f.filter(x => x !== id) : [...f, id]); setFollowerCounts(fc => ({ ...fc, [id]: (fc[id] != null ? fc[id] : USERS[id].followers) + (now ? -1 : 1) })); if (!now) { gainXp(2); setTimeout(() => addNotif({ type: "follow", fromId: id }), 1800); } };

  const onLike = (id) => setPosts(ps => ps.map(p => p.id === id ? { ...p, likedByMe: !p.likedByMe, likes: p.likes + (p.likedByMe ? -1 : 1) } : p));
  const onReact = (id, emoji) => setPosts(ps => ps.map(p => { if (p.id !== id) return p; if (p.reaction === emoji) return { ...p, reaction: null, likedByMe: false, likes: p.likes - 1 }; return { ...p, reaction: emoji, likedByMe: true, likes: p.likedByMe ? p.likes : p.likes + 1 }; }));
  ...
  const onComment = (id, text) => { setPosts(ps => ps.map(p => p.id === id ? { ...p, comments: [...p.comments, { id: "c" + Date.now(), authorId: ME, text, time: "ახლა" }] } : p)); gainXp(5); };
  ...
  const onPost = (text, picked, poll) => { setPosts(ps => [np({ authorId: ME, time: "ახლა", text, image: picked ? img(picked) : null, poll: poll || null, likes: 0 }), ...ps]); setCreateOpen(false); gainXp(15); flash("გამოქვეყნდა 🎉 +15 XP"); const pool = ["u2", "u3", "u5", "u7", "u6", "u4"]; const rnd = () => pool[Math.floor(Math.random() * pool.length)]; setTimeout(() => addNotif({ type: "like", fromId: rnd() }), 2500); setTimeout(() => addNotif({ type: "comment", fromId: rnd(), text: ["მაგარია 🔥", "სუპერ!", "კაი პოსტია 👏", "❤️"][Math.floor(Math.random() * 4)] }), 5200); };
  const onSendMsg = (cid, partial) => setConvos(cs => cs.map(c => c.id === cid ? { ...c, unread: 0, messages: [...c.messages, { id: "m" + Date.now() + Math.round(Math.random() * 999), fromMe: true, time: "ახლა", ...partial }] } : c));
  ...
  const onCreateConvo = (memberIds) => { const id = "cv" + Date.now() + Math.round(Math.random() * 999); const members = [...new Set(memberIds)].filter(x => x !== ME); const isGroup = members.length > 1; const name = isGroup ? members.map(m => USERS[m].name.split(" ")[0]).join(", ") : null; setConvos(cs => [{ id, members, isGroup, name, unread: 0, messages: [] }, ...cs]); return id; };
  const openStory = (id) => setStoryId(id);
  const markSeen = (id) => setStories(s => s.map(x => x.id === id ? { ...x, seen: true } : x));
  const onAddStory = (item) => { setStories(ss => { const has = ss.some(s => s.authorId === ME); return has ? ss.map(s => s.authorId === ME ? { ...s, seen: false, items: [...s.items, item] } : s) : [{ id: "sme", authorId: ME, seen: false, items: [item] }, ...ss]; }); setStoryEditorOpen(false); gainXp(8); flash("Story დაემატა ✨ +8 XP"); };
  ...
  const onReelLike = (id) => setReels(rs => rs.map(r => r.id === id ? { ...r, likedByMe: !r.likedByMe } : r));
  ...
  return (
    <div className="min-h-screen w-full" style={{ color: C.ink, fontFamily: BODY, backgroundColor: C.paper, backgroundImage: `radial-gradient(${C.grid} 1px, transparent 1px)`, backgroundSize: "22px 22px" }}>
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "fadein"} key={tab + (profileId || "") + (activeTag || "")}>
            ...
            {tab === "messages" && <Messages convos={convos} openId={openConvoId} setOpenId={setOpenConvoId} onSend={onSendMsg} onReply={onReply} onCreateConvo={onCreateConvo} onOpenProfile={openProfile} />}
      ...
      {createOpen && <CreateSheet onClose={() => setCreateOpen(false)} onPost={onPost} />}
      ...
      {settingsOpen && <SettingsView settings={settings} setSettings={setSettings} meProfile={meProfile} setMeProfile={setMeProfile} mode={mode} setMode={setMode} onClose={() => setSettingsOpen(false)} flash={flash} />}
      {storyEditorOpen && <StoryEditor onClose={() => setStoryEditorOpen(false)} onShare={onAddStory} />}
...
function SettingsView({ settings, setSettings, meProfile, setMeProfile, mode, setMode, onClose, flash }) {
    ...
    <div className="fixed inset-0 z-[59] flex justify-center" style={{ background: C.paper }}>
      <div className="w-full max-w-[600px] flex flex-col" style={{ height: "100dvh", borderLeft: `1px solid ${C.line}`, borderRight: `1px solid ${C.line}` }}>
        ...
        <div className="flex-1 overflow-y-auto pb-10">
          ...
          <div className="px-4 mt-6"><button onClick={() => flash("გასვლა (demo)")} className="w-full py-3 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 active:scale-[.98]" style={{ background: C.likeSoft, color: C.like }}><LogOut size={18} /> გასვლა</button></div>
...
function StoryEditor({ onClose, onShare }) {
  ...
  return (
    <div className="fixed inset-0 z-[62] flex items-center justify-center" style={{ background: "rgba(0,0,0,.96)" }}>
      <div className="relative w-full max-w-[440px] h-full sm:h-[92vh] sm:rounded-3xl overflow-hidden" style={{ background: "#000" }}>
        <div className="absolute inset-0" style={{ filter }}><Pic src={img(pic, 480, 854)} grad={GRADS[hashIdx(pic, GRADS.length)]} className="w-full h-full" /></div>
        ...
        <div className="absolute bottom-0 inset-x-0 p-3" style={{ background: "linear-gradient(0deg, rgba(0,0,0,.65), transparent)" }}>
          {mode === "filter" && <div className="flex gap-2 overflow-x-auto no-scrollbar mb-3">{FILTERS.map(([l, f]) => <button key={l} onClick={() => setFilter(f)} className="shrink-0 flex flex-col items-center gap-1"><div className="rounded-xl overflow-hidden" style={{ width: 54, height: 54, outline: filter === f ? "2.5px solid #fff" : "none", outlineOffset: 2 }}><div style={{ filter: f, width: "100%", height: "100%" }}><Pic src={img(pic, 96, 96)} className="w-full h-full" /></div></div><span className="text-[10px] text-white">{l}</span></button>)}</div>}
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => onShare({ image: img(pic, 480, 854), filter, text: text.trim(), stickers })} className="rounded-full px-5 py-3 font-bold text-white flex items-center gap-2 shrink-0 active:scale-95" style={{ backgroundImage: GBRAND, boxShadow: SH.glow }}>გაზიარება <Send size={17} /></button>
          </div>
        </div>
      </div>
    </div>
