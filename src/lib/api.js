...
export const comments = {
  ...
  forPosts: async (ids) => {
    const { data, error } = await need().from("comments").select("*, author:profiles(*)").in("post_id", ids).order("created_at");
    if (error) throw error;
    return data;
  },
};
...
export const chat = {
  conversations: async () => {
    const { data, error } = await need()
      .from("conversations")
      .select("*, members:conversation_members(profiles(*)), messages(*)")
      .order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  },
...
export const stories = {
  ...
  create: async ({ image_url, filter, text, stickers }) => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const { data, error } = await sb.from("stories").insert({ author_id: uid, image_url, filter, text, stickers }).select().single();
    if (error) throw error;
    return data;
  },
};
...
export const reels = {
  list: async () => {
    const { data, error } = await need()
      .from("reels")
      .select("*, author:profiles(*), reel_likes(count)")
      .order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  },
